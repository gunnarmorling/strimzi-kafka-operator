...
manage Roles and Role Bindings (RBAC) resources.
...
:context: {ContextProduct}
include::assembly-deploying-cluster-operator.adoc[leveloffset=+1]
...
When the Topic Operator is deployed by the Cluster Operator it will be configured to watch for `KafkaTopics` with the label `strimzi.io/cluster: <cluster-name>`

NOTE: When the Topic Operator is deployed manually the `strimzi.io/cluster` label is not necessary.

The `KafkaTopic` contains the topic configuration in a specific format, described in <<topic_config_map_details>>.
