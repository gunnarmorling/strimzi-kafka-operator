...
`watchedNamespace`::
The {ProductPlatformName} namespace in which the topic operator watches for `KafkaTopics`. Default is the namespace
...
no image is specified as the `spec.topicOperator.image` in the <<topic_operator_json_config,topic operator config>>
